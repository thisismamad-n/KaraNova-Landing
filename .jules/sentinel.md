...
## 2026-03-30 - Overly Permissive Content Security Policy (Supply Chain Attack Risk)
**Vulnerability:** The Content Security Policy in `proxy.ts` and `next.config.js` was overly permissive, allowing `'unsafe-inline'` for scripts and styles in production, and not strictly defining `img-src` and `connect-src`. This could allow a compromised third-party dependency (e.g., an animation library) to inject malicious scripts, manipulate the DOM (defacement), and exfiltrate data.
**Learning:** Relying purely on framework escaping is insufficient if the framework's dependencies themselves are compromised. A strong CSP acts as a defense-in-depth mechanism, explicitly restricting what the browser is allowed to execute or load, even if malicious code is present.
**Prevention:** Implement a strict CSP that removes `'unsafe-inline'` for scripts in production (using nonces for Next.js inline scripts where necessary), strictly defines allowed external domains for connections (`connect-src`) and images (`img-src`), and outright denies framing (`frame-ancestors 'none'`, `frame-src 'none'`).
